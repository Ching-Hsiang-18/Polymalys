...
class DummyOrder {
	protected:
		DummyOrder() { } // prevent instantiation
	public:
...
class PushHelper {
public:
	static inline void push(typename G::vertex_t v, Vector<typename G::vertex_t> &wl_vertices, O *order) {
		int i;
		for (i = 0; i < wl_vertices.count() && order->isBefore(v, wl_vertices[i]); i++);
		wl_vertices.insert(i, v);
	}
};

template <class G>
class PushHelper<G, DummyOrder> {
public:
	static inline void push(typename G::vertex_t v, Vector<typename G::vertex_t> &wl_vertices, DummyOrder *order) {
		wl_vertices.push(v);
	}
};
 ...
 * @param S		Storage.
 * @param O		Optional order (must implement otawa::Order concept)
 */
template <class D, class G, class S, class O = DummyOrder>
class WorkListDriver: public PreIterator<WorkListDriver<D, G, S, O>, typename G::vertex_t > {
public:
	 ...
	 * @param order An order of processing. Must be non-null if the template parameter O was provided.
	 */
	WorkListDriver(D& dom, const G& graph, S& store, O *order = NULL)
	...
	inline void check_join(typename G::edge_t edge, typename D::t s) {
		typename D::t ps = _store.get(edge);
		typename D::t joined = _dom.join(ps, s);
		if(!_dom.equals(joined, ps))
			change(edge, joined);
	}
...
private:
	inline void push(typename G::vertex_t v) {
		if(!wl_set.bit(_graph.index(v))) {
			PushHelper<G,O>::push(v, wl_vertices, _order);
