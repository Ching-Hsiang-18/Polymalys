...
# configuration
OTAWA_REV = "ad8ca1d97631481a6faec2f4a25cb9f92b15408c"
APP = "OTAWA"
...
class GitDownloader(Downloader):
	...
	def download(self, mon):
		...
		if res != 0:
			return False
		if self.address == "https://git.renater.fr/anonscm/git/otawa/otawa.git":
			cmd = "cd %s ; git checkout %s" % (target, OTAWA_REV)
			res = mon.execute(cmd, mon.get_log_file(), mon.get_log_file())
		return res == 0
